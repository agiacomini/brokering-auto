...
public interface IAutoRepository extends JpaRepository<Auto, Long> {
    ...
    // JPQL
    @Query(value = "SELECT a FROM Auto a WHERE a.id = :id AND a.quattroPerQuattro = :quattroPerQuattro " +
            "                            AND a.fasciaDiPrezzo = :fasciaDiPrezzo AND a.anzianita = :anzianita " +
            "                            AND a.cambioAutomatico = :cambioAutomatico AND a.alimentazione = :alimentazione")
    Auto selectTestAuto(@Param("id") Integer id,
                        @Param("quattroPerQuattro") boolean quattroPerQuattro,
                        @Param("fasciaDiPrezzo") String fasciaDiPrezzo,
                        @Param("anzianita") Integer anzianita,
    ...
    List<Auto> selByAnzianitaAndAlimentazione(@Param("anzianita") Integer anzianita,
                                              @Param("cambioAutomatico")
                                              boolean cambioAutomatico);

    // NATIVE QUERY
    @Query(value = "SELECT * FROM auto a WHERE a.anzianita = :anzianita AND a.cambio_automatico = :cambioAutomatico " +
            "                            AND a.fascia_di_prezzo = :fasciaDiPrezzo", nativeQuery = true)
    List<Auto> selectAutoByAnzianitaAndCambioAutomaticoAndFasciaPrezzo(@Param("anzianita") Integer anzianita,
                                                                       @Param("cambioAutomatico") boolean cambioAutomatico,
                                                                       @Param("fasciaDiPrezzo") String fasciaDiPrezzo);

    @Query(value = "SELECT * FROM auto a WHERE a.anzianita = :anzianita AND a.cambio_automatico = :cambioAutomatico " +
            "                            AND a.alimentazione = :alimentazione", nativeQuery = true)
